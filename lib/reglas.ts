 ...
 *
 * ⚠️ El motor AUTORITATIVO vive en Postgres (transaccional, sin condiciones
 * de carrera entre dispositivos):
 *   - turno_entrar_a_cola      → entrar a cola + anti-duplicado (caso 18)
 *   - turno_llamar_siguiente   → siguiente atómico, respeta cita confirmada (1,3,4,8,16)
 *   - turno_confirmar_camino   → "voy en camino" (6)
 *   - turno_eta                → ETA con duraciones reales (10)
 *   - turno_expirar_llamados   → cron: vence llamados/citas (5,13,15)
 *   + triggers: cita→cola_prioritaria (7), visitas, puntos, confiabilidad, grupos (20)
 * Usar siempre los wrappers de lib/db.ts. Estas funciones quedan como
 * helpers de UI / previsualización en el cliente.
 */
...
export function calcularETA(colaDelante: Cola[], tiempoEntreClientes: number): number {
  // suma la duración real de cada servicio delante + el gap entre clientes
  return colaDelante.reduce(
    (total, c) => total + (c.servicio?.duracion_min ?? 0) + tiempoEntreClientes,
    0
  )
}
