...
export default function Resenas({ perfilId, nombre, visible, onClose }: {
  ...
  return (
    <Hoja visible={visible} onClose={onClose}>
      ...
      {cargando ? <ActivityIndicator color={COLORS.ink} style={{ marginVertical: 28 }} /> : (
        <>
...
const s = StyleSheet.create({
  ...
  promedio: { fontFamily: FONTS.monoBold, fontSize: 44, color: COLORS.ink },
  estrellasT: { fontSize: 15, color: COLORS.redText, marginTop: -2 },
  ...
  barraK: { fontFamily: FONTS.bold, fontSize: 12, color: COLORS.textLight, width: 10 },
  barraBg: { flex: 1, height: 8, borderRadius: 4, backgroundColor: GLASS.fillStrong, overflow: 'hidden' },
  barraFill: { height: 8, backgroundColor: COLORS.ink, borderRadius: 4 },
  barraN: { fontFamily: FONTS.regular, fontSize: 12, color: COLORS.textLight, width: 18, textAlign: 'right' },
  vacio: { fontFamily: FONTS.regular, fontSize: 14, color: COLORS.textMid, lineHeight: 19, marginVertical: 18 },
  item: { borderTopWidth: 1, borderTopColor: GLASS.hairline, paddingVertical: 12 },
  itemTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  itemEstrellas: { fontSize: 14, color: COLORS.redText },
  itemMeta: { fontFamily: FONTS.regular, fontSize: 12, color: COLORS.textLight },
  itemTexto: { fontFamily: FONTS.regular, fontSize: 14, color: COLORS.ink, marginTop: 6, lineHeight: 20 },
})
