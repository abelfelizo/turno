...
| `obstaculos.test.sql` | El mismo día pero con fila, agenda y bloqueos **a la vez**. Los fallos que quedaban no estaban en ninguna de las tres piezas: estaban en los cruces. Desde la 116 **le fija el huso al local**: montaba «una cita en curso» de hace cinco minutos a dentro de veinticinco, y corriéndola a las 23:51 esa cita cruzaba la medianoche. Fallaba media hora al día y pasaba las otras veintitrés y media. |
...
| `doble_servicio.test.sql` | **Uno detrás del otro, con dos personas** (migración 114). Antes «doble servicio» era otra cosa: dos turnos a la vez de oficios distintos, en paralelo y sin relación — se podía llamar a la manicura con el cliente sentado en la silla del barbero. Prueba que el segundo no se llama por ninguna de las dos puertas, que un turno bloqueado **no tapa la fila** a los demás, y sobre todo que se suelta solo cuando el primero termina, acabe bien o acabe mal: si no, un corte abandonado dejaría al cliente sin que lo llamen nunca más. |
| `madrugada.test.sql` | **La madrugada también es una jornada** (migración 113). Un local de 9 de la noche a 3 de la mañana —Navidad, Nochevieja, la víspera del Día de las Madres— a la 01:18 leía «ahora está cerrado: su fila abre de 21:00 a 03:00»: un letrero que contiene la hora actual. Una jornada se ancla al día en que EMPIEZA, así que a la 01:18 la viva es la de ayer. Prueba el caso de verdad **a cualquier hora del día** moviendo el huso del local, no retorciendo el horario. |
...
| `fila_a_mano.test.sql` | **La fila se toca por la puerta** (migración 119). Las RPC tienen su portero, pero la tabla estaba abierta por detrás: un cliente podía insertarse con prioridad 1 o marcar su turno como atendido —y el trigger le apuntaba una visita—. Prueba que eso se cierra y que lo que la app hace a mano (salir de la fila, cobrar) sigue funcionando. Solo tiene sentido con la 119 aplicada. |
| `clientes_dueno.test.sql` | **Los clientes del dueño cuentan el local** (migración 120). El administrador ve las visitas de su silla y de sus empleados —no las de quien le renta— y con quién se corta cada cliente; nadie más abre esa función, y la lista del barbero (117) no cambia. Solo tiene sentido con la 120 aplicada. |
